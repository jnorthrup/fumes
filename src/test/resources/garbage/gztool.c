...
enum VERBOSITY_LEVEL {
    VERBOSITY_NONE = 0, VERBOSITY_NORMAL = 1, VERBOSITY_EXCESSIVE = 2,
...
static inline void *endiannes_swap_4p(void *x) {
    *(uint32_t * )
    x = endiannes_swap_4(*(uint32_t * )
    x);
    return x;
}

static inline uint64_t
endiannes_swap_8(uint64_t
v)
{
...
// in each call there's no problem as strm_avail_in0 == CHUNK and so this fread() reads 0 bytes.
                ...
                strm.avail_in = fread(input + strm_avail_in0, 1, CHUNK - strm_avail_in0,
                                      file_in); // here I do not use pread() just to maintain the same code base as in decompress_and_build_index()
...
// has been overwritten from the beginning (possible with rsyslog logs, for example)
            if (strlen(file_name) > 0 &&  // this check cannot be done on STDIN
                expected_first_byte <=
                1LLU // this check must not be done if using `-n` (because then st.st_size < totin in general)
                    ) {
                struct stat st;
                stat(file_name, &st);
...
// decompressor state MUST be reinitiated after Z_STREAM_END
        if (ret.error == Z_STREAM_END) {
            ...
            strm.avail_in = 0;
            strm.next_in = Z_NULL;
            ret.error = inflateInit2(&strm,
                                     47);      /* automatic zlib or gzip decoding (15 + automatic header detection) */
...
// with not Supervising options, a Z_STREAM_END at feof() is correct!
                    if ((feof(file_in) ||
                        ...
                        && strm.avail_in == 0) {
                        printToStderr(VERBOSITY_MANIAC,
                                      "Correct END OF GZIP file detected at EOF whilst 'in advance' mode.\n");
...
/* window is compressed on memory, so decompress it */
            ...
            (void) inflateSetDictionary(&strm, decompressed_window,
                                        local_window_size); // (local_window_size may not be WINSIZE)
...
// end of bgzip-compatible-streams code
        ...
        printToStderr(VERBOSITY_CRAZY,
                      "output_data_counter=%llu,totin=%llu,totout=%llu,totlines=%llu,ftello=%llu,avail_in=%d\n",
...
// and now this variable is not needed anymore.
                ...
                if (gzip_eof_detected == 0) {
                    if (end_on_first_proper_gzip_eof == 1 &&
                        ...
                        gzip_eof_detected = 1;
                        printToStderr(VERBOSITY_NORMAL,
                                      "Warning: GZIP end detected in the middle of data: deactivating `-E`\n");
                        end_on_first_proper_gzip_eof = 0;
                    }
                }
                if (offset_in >= 8) {
...
// if necessary:
                            ...
                            if (fwrite(window + offset + (WINSIZE - avail_out_0), 1, have - offset, file_out) !=
                                (have - offset) ||
                                ferror(file_out)) {
                                (void) inflateEnd(&strm);
                                ret.error = Z_ERRNO;
...
// we must addpoint() from now on :
                ...
                if (actual_index_point == 0 &&
                    ...
                    if (write_index_to_disk ==
                        1) { // if `-W`, index is not written to disk, and it will also not be created/updated (!)

                        if (NULL != index)
                            printToStderr(VERBOSITY_EXCESSIVE,
                                          "addpoint index->have = %llu, index_last_written_point = %llu\n",
                                          index->have, index_last_written_point);
                        else
                            printToStderr(VERBOSITY_EXCESSIVE,
                                          "addpoint index->have = 0, index_last_written_point = %llu\n",
                                          index_last_written_point);
...
// free strm structure from internal state of decompress_in_advance():
    decompress_in_advance(NULL, NULL, NULL,
                          0LLU, 0, 0, 0, 0, 0, 0LLU, 0LLU, 0, false, 0LLU,
                          DECOMPRESS_IN_ADVANCE_RESET); // returned value is always ok when called with this value
...
// write index point
            if (write_index_to_disk ==
                ...
                index = addpoint(index, 0,
                                 ((0 == totout) ? GZIP_HEADER_SIZE_BY_ZLIB
                                                : totout), // compressed byte after header <=> uncompressed byte 0
...
// as no previous index is possible, so there's no doubt.
        ...
        ret = compress_and_build_index(file_in, file_out, compression_factor,
                                       ...
                                       write_index_to_disk,
                                       ((end_on_first_proper_gzip_eof == 0) ? 1
                                                                            : 0), // `-E` behaviour is inverted with `-c` for ease of use
...
// otherwise decompression cannot proceed
            ...
            ret = decompress_and_build_index(file_in, file_out, file_name, span_between_points, index,
                                             DECOMPRESS, offset, line_number_offset, index_filename,
                                             0, // write_index_to_disk = 0
                                             1, always_create_a_complete_index, // end_on_first_proper_gzip_eof = 1
                                             waiting_time, extend_index_with_lines,
                                             1, // expected_first_byte not used, so 1
...
// use last available index point as data for percentage
                if (index->have > 1 && // index has at least one useful index point
                    ...
                    fprintf(stdout, "%.2f%%\n",
                            100.0 -
                            (double) index->list[index->have - 1].in / (double) index->list[index->have - 1].out *
                            100.0);
                } else {
            ...
            for (j = 0; j < index->have; j++) {
                if (list_verbosity == VERBOSITY_EXCESSIVE &&
                    ...
                    fprintf(stdout, "#%llu: @ %llu / %llu ",
                            (long long unsigned) (j + 1), (long long unsigned) index->list[j].in,
                            (long long unsigned) index->list[j].out);
                    if (1 == index->index_version) // print line number information
                        fprintf(stdout, "L%llu ", (long long unsigned) index->list[j].line_number);
                    fprintf(stdout, "( %d @%llu ), ", index->list[j].window_size,
                            (long long unsigned) index->list[j].window_beginning);
                    if ((j + 1) % 5 == 0) {
                        fprintf(stdout, "\n");
                    }
                }
...
/* window is compressed on memory, so decompress it */
                    ...
                    if (verbosity_level > VERBOSITY_NONE) {
                        ...
                        fprintf(stdout, "( %d/%llu %.2f%% ), ", index->list[j].window_size,
                                ...
                                ((local_window_size > 0) ? (100.0 - (double) (index->list[j].window_size) /
                                                                    (double) local_window_size * 100.0) : 0.0));
                        if ((j + 1) % 5 == 0) {
                            fprintf(stdout, "\n");
                        }
                    }
...
// `-b #` extracts data from indicated position byte in uncompressed stream of <FILE>
            case 'b':
                extract_from_byte = (uint64_t)
                giveMeAnInteger(optarg);
...
// Accepts SI suffixes.
            case 'r':
                range_number_of_bytes = (uint64_t)
                giveMeAnInteger(optarg);
...
// Accepts SI suffixes.
            case 'R':
                range_number_of_lines = (uint64_t)
                giveMeAnInteger(optarg);
...
// if no index filename is set (`-I`), compression cannot proceed
            printToStderr(VERBOSITY_NORMAL,
                          "ERROR: Index filename must be provided with `-I` when compressing STDIN.\n");
            return EXIT_GENERIC_ERROR;
        }
...
printToStderr( VERBOSITY_NORMAL, "WARNING: `-[Xx]` will be ignored because index already exists.\n" );*/
            ...
            if (force_action == 0) {
                if (action == ACT_COMPRESS_AND_CREATE_INDEX) {
                    ...
                    } else { ; //printToStderr( VERBOSITY_NORMAL, "(Use `-f` to force overwriting.)\n" );
                    }
                }
            } else {
                if (write_index_to_disk == 1) {
...
// `-F` has no sense with stdin
        if (force_strict_order == 1) {
            printToStderr(VERBOSITY_NORMAL,
                          "WARNING: There is no sense in using `-F` with STDIN input: ignoring `F`.\n");
...
// stdin is a gzip file
                if (index_filename_indicated == 1) {
                    ...
                    printToStderr(VERBOSITY_NORMAL,
                                  "ERROR: `-I INDEX` must be used when extracting from STDIN.\nAborted.\n");
                    ret_value = EXIT_GENERIC_ERROR;
                    break;
                }
...
// as stdout is already used for data extraction
                    printToStderr(VERBOSITY_NORMAL,
                                  "ERROR: Index filename is needed if STDIN is used as gzip input.\nAborted.\n");
